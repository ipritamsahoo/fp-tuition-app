...
export default function ProfilePicture({ size = 36, picUrl, name, className = "", showBadge = true }) {
    ...
    const initialFallback = (
        <div
            className="rounded-full bg-gradient-to-tr from-[#3861fb] to-[#2b4fcf] flex items-center justify-center text-white font-bold shadow-sm w-full h-full select-none"
    ...
    return (
        <div 
            ...
            style={{ width: outerSize, height: outerSize, isolation: "isolate", transform: "translateZ(0)" }}
            onDoubleClick={(e) => { e.preventDefault(); e.stopPropagation(); }}
            onMouseDown={(e) => { if (e.detail > 1) e.preventDefault(); }}
        >
